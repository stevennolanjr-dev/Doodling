...
// Configuration - set to true for viewer version
...
export default function MobilityAllocationTool() {
  ...
  const loadInitialState = (key, fallback) => {
    if (READ_ONLY) return fallback; // Viewer loads from props/fetch
    try {
  ...
  const [units, setUnits] = useState(() => loadInitialState('units', initialUnits));
  const [commitments, setCommitments] = useState(() => loadInitialState('commitments', initialCommitments));
  const [tdgEntries, setTdgEntries] = useState(() => loadInitialState('tdgEntries', initialTDG));
  const [wingInventory, setWingInventory] = useState(() => loadInitialState('wingInventory', initialWingInventory));
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = `mobility-allocation-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
  ...
  const resetData = () => {
    ...
    setTdgEntries(initialTDG);
    setWingInventory(initialWingInventory);
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0f1a', color: '#e2e8f0', fontFamily: "'JetBrains Mono', 'SF Mono', monospace", fontSize: '12px', userSelect: dragState ? 'none' : 'auto' }}>
      ...
      <div style={{ padding: '12px 20px', borderBottom: '1px solid #1e293b', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(180deg, #0f172a 0%, #0a0f1a 100%)' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '16px', fontWeight: 600, letterSpacing: '0.05em', color: '#f8fafc' }}>MOBILITY FORCE ALLOCATION {READ_ONLY && <span style={{ fontSize: '10px', color: '#64748b', fontWeight: 400 }}>(VIEW ONLY)</span>}</h1>
          <p style={{ margin: '2px 0 0 0', color: '#64748b', fontSize: '10px' }}>AMC A38 • FY24-FY30 • {unitCounts.total} units{showTheaterAssets && ` + ${unitCounts.theater} Theater`}{lastSaved && !READ_ONLY && ` • Saved ${lastSaved.toLocaleTimeString()}`}</p>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          {!READ_ONLY && (
            <>
              <button onClick={() => fileInputRef.current?.click()} style={{ padding: '6px 10px', backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '4px', color: '#94a3b8', cursor: 'pointer', fontSize: '10px', fontFamily: 'inherit' }} title="Import JSON">📥</button>
              <button onClick={exportData} style={{ padding: '6px 10px', backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '4px', color: '#94a3b8', cursor: 'pointer', fontSize: '10px', fontFamily: 'inherit' }} title="Export JSON">📤</button>
              <button onClick={resetData} style={{ padding: '6px 10px', backgroundColor: '#1e293b', border: '1px solid #7f1d1d', borderRadius: '4px', color: '#f87171', cursor: 'pointer', fontSize: '10px', fontFamily: 'inherit' }} title="Reset to defaults">⟲</button>
            </>
          )}
          ...
          {!READ_ONLY && <button onClick={() => setEditMode(!editMode)} style={{ padding: '6px 12px', backgroundColor: editMode ? '#1d4ed8' : '#1e293b', border: editMode ? '1px solid #3b82f6' : '1px solid #334155', borderRadius: '4px', color: '#f8fafc', cursor: 'pointer', fontSize: '10px', fontWeight: 500, fontFamily: 'inherit' }}>{editMode ? '✓ EDIT' : 'VIEW'}</button>}
        </div>
      </div>
      ...
      <div style={{ padding: '8px 20px', borderBottom: '1px solid #1e293b', backgroundColor: '#0f172a', display: 'flex', gap: '8px', alignItems: 'center', overflowX: 'auto' }}>
        ...
        {!READ_ONLY && editMode && <button onClick={() => setTdgModal({ mode: 'view' })} style={{ marginLeft: 'auto', padding: '4px 8px', backgroundColor: '#422006', border: '1px solid #f59e0b', borderRadius: '4px', color: '#fcd34d', cursor: 'pointer', fontSize: '9px', fontFamily: 'inherit' }}>📋 TDG</button>}
      </div>
                ...
                <React.Fragment key={unit.id}>
                  ...
                  <div style={{ display: 'flex', backgroundColor: isTheater ? '#0d1520' : (idx % 2 === 0 ? '#0a0f1a' : '#0d1117') }}>
                    ...
                    <div style={{ flex: 1, height: ROW_HEIGHT, position: 'relative', borderBottom: '1px solid #0f172a', cursor: editMode && !isTheater ? 'crosshair' : 'default' }} onClick={e => { if (e.target === e.currentTarget) handleTimelineClick(e, unit); }}>
                      ...
                      {!isTheater && unitCommitments.map(c => { const style = getCommitmentStyle(c); if (!style) return null; const hasRFS = c.allocations.some(a => a.type === 'RFS'); const hasRFF = hasRFFAllocation(c); const width = parseFloat(style.width) - 8; const isDragging = dragState?.commitmentId === c.id; return (<div key={c.id} onMouseDown={e => !READ_ONLY && handleDragStart(e, c, unit)} onClick={e => { e.stopPropagation(); if (!dragState && editMode && !READ_ONLY) setSelectedCommitment(c); }} onDoubleClick={e => { e.stopPropagation(); if (!READ_ONLY) { setEditMode(true); setSelectedCommitment(c); } }} onMouseEnter={e => handleMouseEnter(e, c)} onMouseLeave={() => !dragState && setHoveredCommitment(null)} style={{ position: 'absolute', top: afforgenSettings.displayMode === 'stripes' && unit.component === 'A' ? '6px' : '5px', height: afforgenSettings.displayMode === 'stripes' && unit.component === 'A' ? ROW_HEIGHT - 12 : ROW_HEIGHT - 10, borderRadius: hasRFS && !c.isConversion ? '10px' : '3px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '8px', fontWeight: 600, color: c.isConversion ? '#7f1d1d' : '#fff', cursor: editMode && !READ_ONLY ? (isDragging ? 'grabbing' : 'grab') : 'pointer', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', padding: '0 4px', boxShadow: selectedCommitment?.id === c.id ? '0 0 0 2px #fbbf24' : hasRFF ? 'inset 0 0 0 2px rgba(245, 158, 11, 0.8)' : 'none', textShadow: c.isConversion ? 'none' : '0 1px 2px rgba(0,0,0,0.5)', opacity: isDragging ? 0.8 : 1, zIndex: isDragging ? 100 : 1, ...style }}>{getBlockLabel(c, width)}</div>); })}
                    </div>
                  </div>
                </React.Fragment>
              );
